...
// ✅ PASSO 1: IMPORTAR O HOOK DO GOOGLE E O SERVIÇO DE AUTENTICAÇÃO
...
export default function LoginPage() {
  ...
  const { login, handleAuthentication, isAuthenticated, isLoading } = useAuth();
  ...
  const {
    ...
    async (formData) => {
      ...
      // A função 'login' do seu AuthContext já lida com o resultado
      await login(formData);
      setIsSubmitting(false);
    }
  );

  // ✅ PASSO 2: ADICIONAR A LÓGICA DO HOOK DO GOOGLE AQUI
  const handleGoogleLogin = useGoogleLogin({
    flow: 'auth-code', // Crucial para obter o código para o backend
    onSuccess: async (codeResponse) => {
      setIsSubmitting(true);
      toast.loading('Verificando com o Google...');
      ...
      if (response.success && response.data) {
        // ✅ CORREÇÃO: Chamar a nova função que apenas seta o estado, sem fazer outra chamada de API
        handleAuthentication(response.data);
      } else {
        toast.error(response.message || 'Falha no login com Google');
      }
    ...
    <>
      ...
      <Layout variant="auth">
        ...
        >
          ...
          {/* ✅ PASSO 3: ATUALIZAR O BOTÃO DO GOOGLE PARA CHAMAR O HANDLER CORRETO */}
          <Button
            variant="ghost"
            size="lg"
            onClick={() => {
              ...
              handleGoogleLogin();
              // alert('Botão Google Clicado!'); // Você pode usar o alert para testar novamente
            }}
            className="w-full border border-white/20 hover:border-white/40 mb-6"
            disabled={isSubmitting}
          >
            {/* O conteúdo agora está dentro de um span */}
