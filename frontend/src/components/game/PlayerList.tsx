    ...
    return (
      <div className="h-full bg-medieval-800/30 border border-medieval-600 rounded-lg p-4">
        <div className="animate-pulse">
          <div className="bg-medieval-700 h-6 rounded mb-4"></div>
          <div className="space-y-2">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-medieval-700 h-8 rounded"></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        ...
        {spectators.length > 0 && (
          <div>
            ...
            <div className="space-y-1">
              {spectators.map((spectatorId, index) => {
                const spectator = gameState.players.find(p => p.id === spectatorId || p.userId === spectatorId);
                const name = spectator?.username || `Espectador ${index + 1}`;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-blue-300 truncate">{name}</div>
