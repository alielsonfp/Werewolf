// 🐺 LOBISOMEM ONLINE - Action Manager (CORRIGIDO DEFINITIVAMENTE + TRATAMENTO DE ERROS)
...
export class ActionManager {
  ...
  async performAction(playerId: string, actionData: any): Promise<ActionResult> {
    try {
      ...
      // ✅ CORREÇÃO CRÍTICA: Tratamento robusto de erros
      ...
      return {
        success: false,
        actionId: '',
        message: 'Erro interno ao processar ação. Tente novamente.',
  ...
  private async processNightAction(action: GameAction): Promise<ActionResult> {
    try {
      ...
      // ✅ CORREÇÃO: Tratamento robusto de erros em ações noturnas
      ...
      return {
        success: false,
        actionId: action.id,
        message: 'Erro ao processar ação noturna',
  ...
  private async processVote(action: GameAction): Promise<ActionResult> {
    try {
      ...
      // ✅ CORREÇÃO: Tratamento robusto de erros em votação
